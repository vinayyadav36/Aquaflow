    ...
    <div className="space-y-3 mb-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
          ...
          <span className="text-base font-bold text-gray-900">${summaries.cashOutTotal.toFixed(2)}</span>
        </div>
        <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-1.5 mb-1">
          ...
          <span className="text-base font-bold text-gray-900">${summaries.digitalOutTotal.toFixed(2)}</span>
        </div>
      </div>
      ...
      {summaries.topCategory && (
        <div className="flex items-center gap-2 text-xs text-gray-500 bg-white px-3 py-2 rounded-xl border border-gray-100 shadow-sm">
          ...
          <span>Top category: <strong className="text-gray-800 capitalize">{summaries.topCategory.name}</strong> (${summaries.topCategory.amount.toFixed(2)})</span>
        </div>
      )}
    </div>
  );
